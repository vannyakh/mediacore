...
def _process_download(job_id: str) -> None:
    ...
    with SessionLocal() as db:
        ...
        try:
            ...
            public = storage.public_url(job.id, result.path.name)
            db.add(
                Download(
                    job_id=job.id,
...
def _ensure_source(job: Job, storage: LocalStorage) -> Path:
    ...
    with SessionLocal() as db:
        ...
        try:
            if operation == "subtitles":
                ...
                out.write_text(json.dumps(subs, indent=2), encoding="utf-8")
                job.result_path = str(out)
                job.result_url = storage.public_url(job.id, out.name)
                job.status = JobStatus.completed
                job.completed_at = datetime.now(timezone.utc)
                db.add(job)
                db.commit()
            ...
            if not ffmpeg_available() and operation in {"audio", "thumbnail", "convert", "clip"}:
                raise FFmpegError("ffmpeg plugin requires ffmpeg binary")
            ...
            job.result_url = storage.public_url(job.id, dest.name)
            job.status = JobStatus.completed
            job.completed_at = datetime.now(timezone.utc)
            job.error = None
            db.add(job)
            db.commit()
            get_event_bus().emit(EventType.COMPLETED, job_id=job.id, operation=operation)
        except (MediaCoreError, FFmpegError) as exc:
