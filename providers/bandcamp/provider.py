"""Bandcamp provider using public oEmbed (metadata only)."""
...
OEMBED_ENDPOINT = "https://bandcamp.com/oembed"
...
class BandcampProvider(Provider):
    ...
    def metadata(self, url: str) -> MediaMetadata:
        ...
        return metadata_from_oembed(self.name, url, data, title_fallback="Bandcamp track")
    ...
    def download(self, url: str, format_id: str, dest: Path) -> DownloadResult:
        raise ProviderNotConfiguredError(
            f"{self.name} (download requires authorized Bandcamp / content-owner access)"
        )
