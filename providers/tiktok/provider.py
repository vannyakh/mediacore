"""TikTok provider using public oEmbed (metadata only)."""
...
class TiktokProvider(Provider):
    ...
    def supports(self, url: str) -> bool:
        ...
        return host in {
            ...
            "vm.tiktok.com",
            "vt.tiktok.com",
        } or (bool(host) and host.endswith(".tiktok.com"))

    def metadata(self, url: str) -> MediaMetadata:
        data = fetch_oembed(OEMBED_ENDPOINT, url, provider_name=self.name)
        return metadata_from_oembed(self.name, url, data, title_fallback="TikTok video")
    ...
    def download(self, url: str, format_id: str, dest: Path) -> DownloadResult:
        raise ProviderNotConfiguredError(
            f"{self.name} (download requires TikTok API / authorized access)"
        )
