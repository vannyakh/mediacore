"""Streamable provider using public oEmbed (metadata only)."""
...
class StreamableProvider(Provider):
    name = "streamable"
    ...
    def supports(self, url: str) -> bool:
        host = hostname(url)
        return host in {"streamable.com", "www.streamable.com"}

    def metadata(self, url: str) -> MediaMetadata:
        data = fetch_oembed(OEMBED_ENDPOINT, url, provider_name=self.name)
        return metadata_from_oembed(self.name, url, data, title_fallback="Streamable video")
    ...
    def download(self, url: str, format_id: str, dest: Path) -> DownloadResult:
        raise ProviderNotConfiguredError(
            f"{self.name} (download requires authorized Streamable access)"
        )
